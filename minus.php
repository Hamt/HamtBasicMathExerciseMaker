<html>
	<head>
		...
		<style>
			...
			p.number.bottom{ border-bottom:2px solid black; background: url(/quiz_assets/minus.png) no-repeat left 2px;background-size:14px; padding:0px 0px 10px 0px;}
			p.answer{ color:red; }
	...
	<body>
		<a href='/minus.php'><h1>여러자리 뺄셈 연습</h1></a>
		<hr />
		<div id='setting-wrap'>
		<?php
		...
		$pagekey = 'minus_'.md5(date('YmdHis').rand(100000, 999999));
		...
		<div id='setting' style='text-align:center;'>
			...
			<?php if(isset($_GET['save_file']) && $_GET['save_file'] == 'Y') : ?> 
				답안지 페이지 : http://hamt.kr/minus.php?pagekey=<?=urlencode($pagekey)?>
				<br />
